...
// Confirm-each-time (Echo adapter) tests. The refusal semantics and the 403
// body shape live in the core package; these tests pin the ADAPTER contract:
// a confirmation_required refusal ends the Echo chain with the confirmation
// link intact (through Echo's HTTPError handler), a malformed ceremony still
// fails closed, the *WithOptions variants carry the digest and summary
// without costing c.Bind its body, and an accepted retry exposes the consumed
// ceremony via GetActionConfirmation.
...
func confirmingVariants(client *agentadmit.Client) map[string]echo.MiddlewareFunc {
...
func TestMiddleware_ConfirmationRequiredMalformedFailsClosed(t *testing.T) {
	...
	e.POST("/api/payments", func(c echo.Context) error {
		invoked = true
		return c.String(http.StatusOK, "ok")
	}, MiddlewareWithOptions(client, payOptions(), "write:payments"))
...
// summary ride along, c.Bind still sees the full body, and the consumed
// ceremony is readable via GetActionConfirmation.
...
func TestMiddleware_PlainRouteForwardsAttestationOnly(t *testing.T) {
	...
	e.POST("/api/payments", func(c echo.Context) error {
		consumed = GetActionConfirmation(c)
		return c.String(http.StatusOK, "ok")
	}, Middleware(client, "write:payments"))

...
// The echo adapter must send the same per-call telemetry as the core
...
// An active response carrying error: bound_exceeded is a refusal — the echo
// adapter must return 403 and never run the handler.
func TestMiddleware_ActiveBoundExceeded403(t *testing.T) {
	...
	e.POST("/pay", func(c echo.Context) error {
		invoked = true
		return c.String(http.StatusOK, "ok")
	}, Middleware(client, "pay:invoices"))
